...
async function webauthntest_register() {
	const serverChallenge: PublicKeyCredentialCreationOptionsJSON = await (await window.fetch(
	...
	userIdElement.value = serverChallenge.user.id;
	...
	// }
	const registerResponseRequest = await startRegistration(serverChallenge);
	console.log(registerResponseRequest);
	const result = await (await window.fetch(
		'/rp/register-response',
		{
			method: "POST",
			headers: {"Content-Type": "application/json"},
			body: JSON.stringify({response: registerResponseRequest, userId: serverChallenge.user.id}),
		}
	)).json();
	console.log({result});
}
...
async function webauthntest_auth() {
	...
	const serverChallenge: PublicKeyCredentialRequestOptionsJSON = await (await window.fetch(
		'/rp/auth-challenge',
		{
			method: "POST",
			headers: {"Content-Type": "application/json"},
	...
	console.log({serverChallenge});
	const cred = await startAuthentication(serverChallenge);
